...
export const useActivityLoader = (currentDog: Dog | null) => {
  ...
  const hasMigrated = useCallback((dogId: string, type: 'scheduled' | 'user') => {
    const flagKey = `migrated_${type}_${dogId}`;
    return migrationFlags[flagKey] || localStorage.getItem(flagKey) === 'true';
  }, [migrationFlags]);

  // Mark migration as completed
  const markMigrated = useCallback((dogId: string, type: 'scheduled' | 'user') => {
    const flagKey = `migrated_${type}_${dogId}`;
    localStorage.setItem(flagKey, 'true');
    setMigrationFlags(prev => ({ ...prev, [flagKey]: true }));
  }, []);

  const migrateScheduledActivity = useCallback((activity: any): ScheduledActivity => {
    return {
      ...
      scheduledDate: activity.scheduledDate || new Date().toISOString().split('T')[0],
  ...
  const loadActivitiesFromSupabase = useCallback(async (
    ...
    try {
      ...
      if (supabaseData.status === 'fulfilled' && supabaseData.value) {
        const { user: userActivities, discovered: discoveredActivities } = supabaseData.value;
        
        // Load scheduled activities from repository
        const scheduledActivities = await ActivityRepository.getScheduledActivities(currentDog.id);
