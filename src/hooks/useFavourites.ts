    ...
    try {
      ...
      const enrichedFavourites = data.map(fav => {
        ...
        if (fav.activity_type === 'library') {
          activityDetails = getPillarActivities().find(a => a.id === fav.activity_id);
        } else if (fav.activity_type === 'user') {
          activityDetails = userActivities.find(a => a.id === fav.activity_id);
        } else if (fav.activity_type === 'discovered') {
          activityDetails = discoveredActivities.find(a => a.id === fav.activity_id);
        }
