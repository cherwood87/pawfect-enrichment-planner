...
export const useActivityValidation = () => {
	...
	const validateScheduledActivity = useCallback(
		(
			...
			if (!dogValidation.isValid) {
				console.error(
					"❌ [useActivityValidation] Dog validation failed:",
					dogValidation.errors,
				);
				toast({
			...
			console.log(
				"📋 [useActivityValidation] Normalized activity data:",
				normalizedActivity,
			);

			// Comprehensive validation
			const validation = SchedulingValidator.validateScheduledActivity(
				normalizedActivity,
				currentDog,
			);
			if (!validation.isValid) {
				console.error(
					"❌ [useActivityValidation] Activity validation failed:",
					validation.errors,
				);
				toast({
			...
			if (validation.warnings.length > 0) {
				console.warn(
					"⚠️ [useActivityValidation] Activity validation warnings:",
					validation.warnings,
				);
				toast({
					title: "Warning",
			...
			if (duplicateCheck.isDuplicate) {
				console.warn(
					"⚠️ [useActivityValidation] Duplicate activity detected:",
					duplicateCheck,
				);
				toast({
					title: "Activity Already Scheduled",
					description:
						duplicateCheck.message || "This activity is already scheduled",
					variant: "destructive",
				});
				return { isValid: false };
			}
			...
			return {
				isValid: true,
				normalizedActivity: normalizedActivity as ScheduledActivity,
			};
		},
		[toast],
	);
	...
	const validateUserActivity = useCallback(
		(
			...
			isValid: boolean;
			sanitizedActivity?: Omit<UserActivity, "id" | "createdAt" | "dogId">;
		} => {
			if (!currentDog) {
				toast({
					title: "No dog selected",
			...
			const sanitizedActivity = {
				...
				benefits: activity.benefits
					? DataValidator.sanitizeInput(activity.benefits)
					: activity.benefits,
	...
	const validateActivityUpdate = useCallback(
		(updates: Partial<ScheduledActivity>, currentDog: Dog | null): boolean => {
			if (!currentDog) {
				toast({
					title: "No dog selected",
					description: "Please select a dog first",
	...
	const validateCompletionNotes = useCallback(
		(completionNotes?: string): boolean => {
			if (completionNotes) {
