...
export const useScheduledActivityActions = (
	...
	const { refreshScheduledActivities } = useActivityRefresh(
		setScheduledActivities,
		currentDog,
	);

	const addScheduledActivity = useCallback(
		async (activity: Omit<ScheduledActivity, "id">): Promise<void> => {
			console.log(
				"🎯 [useScheduledActivityActions] Starting addScheduledActivity with:",
				{
			...
			const validation = validateScheduledActivity(
				activity,
				currentDog,
			...
			try {
				console.log(
					"💾 [useScheduledActivityActions] Creating scheduled activity via domain service...",
				);
				const createdActivity =
					await ActivityDomainService.createScheduledActivity({
						...
						completionNotes:
							validation.normalizedActivity.completionNotes || "",
						reminderEnabled:
							validation.normalizedActivity.reminderEnabled ?? false,
					} as Omit<ScheduledActivity, "id">);

				console.log(
					"✅ [useScheduledActivityActions] Activity created successfully:",
					{
				...
				console.log(
					"🔄 [useScheduledActivityActions] Refreshing activities list...",
				);
				...
				console.error(
					"❌ [useScheduledActivityActions] Failed to create scheduled activity:",
					error,
				);
	...
	const toggleActivityCompletion = useCallback(
		async (activityId: string, completionNotes?: string): Promise<void> => {
			if (!currentDog) {
				toast({
			...
			try {
				await ActivityDomainService.toggleActivityCompletion(
					...
					currentDog.id,
					completionNotes
						? DataValidator.sanitizeInput(completionNotes)
						: undefined,
				);
				await refreshScheduledActivities();
			} catch (error) {
	...
	const updateScheduledActivity = useCallback(
		async (
			...
			try {
				await ActivityDomainService.updateScheduledActivity(
					activityId,
					currentDog?.id,
					normalizedUpdates,
				);
