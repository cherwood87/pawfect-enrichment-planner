...
export const useDiscoveryOperations = (
  ...
  const getCombinedActivityLibrary = useCallback((): (ActivityLibraryItem | DiscoveredActivity)[] => {
  ...
  }, [discoveredActivities, discoveryConfig, currentDog, setDiscoveredActivities]);

  const approveDiscoveredActivity = useCallback(async (activityId: string): Promise<void> => {
    if (!currentDog) return;
    
    const updated = await DiscoveryService.approveActivity(activityId, discoveredActivities, currentDog.id);
    setDiscoveredActivities(updated);
  }, [discoveredActivities, currentDog, setDiscoveredActivities]);

  const rejectDiscoveredActivity = useCallback(async (activityId: string): Promise<void> => {
    if (!currentDog) return;
    
    const updated = await DiscoveryService.rejectActivity(activityId, discoveredActivities, currentDog.id);
    setDiscoveredActivities(updated);
  }, [discoveredActivities, currentDog, setDiscoveredActivities]);

  const updateDiscoveryConfig = useCallback(async (updates: Partial<ContentDiscoveryConfig>): Promise<void> => {
    if (!currentDog) return;
    
    const updated = await DiscoveryService.updateDiscoveryConfig(currentDog.id, discoveryConfig, updates);
  ...
  const checkAndRunAutoDiscovery = useCallback(async (): Promise<void> => {
    if (!currentDog || !DiscoveryService.shouldRunAutoDiscovery(discoveryConfig)) return;
