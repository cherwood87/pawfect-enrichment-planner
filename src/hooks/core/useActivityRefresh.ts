...
export const useActivityRefresh = (
  ...
  const refreshScheduledActivities = useCallback(async () => {
    ...
    console.log('🔄 [useActivityRefresh] Refreshing scheduled activities for dog:', currentDog.name);
    
    try {
      const activities = await ActivityDomainService.getScheduledActivitiesForDog(currentDog.id);
      console.log('✅ [useActivityRefresh] Refreshed activities:', activities.length, 'activities found');
      setScheduledActivities(activities);
    } catch (error) {
      console.error('❌ [useActivityRefresh] Failed to refresh scheduled activities:', error);
