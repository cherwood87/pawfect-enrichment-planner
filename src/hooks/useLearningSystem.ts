...
import { UserInteraction, ActivityFeedback, SmartRecommendation } from '@/types/learning';
...
export const useLearningSystem = () => {
  ...
  const submitFeedback = useCallback(async (
    ...
    feedback: Partial<Pick<ActivityFeedback, 'rating' | 'difficultyRating' | 'engagementRating' | 'enjoymentRating' | 'feedbackText' | 'wouldRecommend' | 'tags'>>
  ) => {
    if (!user || !currentDog) return;
    ...
    try {
      ...
      toast({
        ...
        description: "Thank you for your feedback! This helps us improve recommendations.",
      });
    } catch (error) {
  ...
  const getDailyRecommendations = useCallback(async (): Promise<SmartRecommendation[]> => {
    if (!user || !currentDog) return [];

    try {
      return await RecommendationService.getDailyRecommendations(user.id, currentDog.id);
    } catch (error) {
  ...
  const updatePillarPreferences = useCallback(async () => {
    ...
    try {
      const preferences = await LearningService.calculatePillarPreferences(user.id, currentDog.id);
      
      if (preferences.length > 0) {
        const pillarWeights = preferences.reduce((acc, pref) => {
