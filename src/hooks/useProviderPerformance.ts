...
export const useProviderPerformance = (providerName: string) => {
  ...
  useEffect(() => {
    ...
    if (rerenderCountRef.current > 1) {
      console.log(`🔄 ${providerName} Provider re-rendered (${rerenderCountRef.current} times)`);
  ...
  useEffect(() => {
    if (metrics.loadTime > 2000) {
      console.warn(`⚠️ ${providerName} Provider took ${metrics.loadTime}ms to load (slow)`);
    }
    
    if (metrics.rerenderCount > 10) {
      console.warn(`⚠️ ${providerName} Provider has re-rendered ${metrics.rerenderCount} times (excessive)`);
    }
    
    if (metrics.errorCount > 3) {
      console.warn(`⚠️ ${providerName} Provider has ${metrics.errorCount} errors (unstable)`);
    }
  ...
  return {
    metrics,
    trackError,
    isPerformant: metrics.loadTime < 1000 && metrics.rerenderCount < 5 && metrics.errorCount === 0
  };
};
