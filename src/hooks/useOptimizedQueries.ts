...
export const useOptimizedQueries = () => {
  ...
  const fetchDashboardData = useCallback(
    async (dogId: string): Promise<DashboardData | null> => {
      ...
      try {
        const data = await queryService.getDashboardData(dogId, {
          useCache: true,
        });
        return data;
      } catch (err) {
  ...
  const fetchWeeklyPlannerData = useCallback(
    async (
      dogId: string,
      startDate: Date,
      endDate: Date,
      ...
      try {
        const data = await queryService.getWeeklyPlannerData(
          dogId,
          startDate,
          endDate,
          { useCache: true },
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Failed to fetch weekly planner data";
        setError(errorMessage);
  ...
  const fetchActivityLibrary = useCallback(
    async (
      options: {
        pillar?: string;
        difficulty?: string;
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Failed to fetch activity library";
        setError(errorMessage);
  ...
  const createActivitiesBatch = useCallback(
    async (
      ...
      try {
        const data =
          await queryService.createScheduledActivitiesBatch(activities);
        return data;
      } catch (err) {
        const errorMessage =
          err instanceof Error ? err.message : "Failed to create activities";
        setError(errorMessage);
  ...
  const clearDogCache = useCallback(
    (dogId: string) => {
