...
export const useActivityPreloading = (
  ...
  const preloadActivity = useCallback((activity: ActivityLibraryItem | DiscoveredActivity) => {
    ...
    if ('materials' in activity && activity.materials) {
      activity.materials.forEach(material => {
        ...
        if (typeof material === 'string' && material.match(/\.(jpg|jpeg|png|gif|webp)$/i)) {
          const img = new Image();
          img.src = material;
        }
      });
  ...
  const preloadNextBatch = useCallback((currentIndex: number) => {
    const startIndex = currentIndex + 1;
    const endIndex = Math.min(startIndex + config.preloadCount, activities.length);
  ...
  }, [config.enabled, config.intersectionThreshold, activities, preloadActivity, preloadNextBatch]);
