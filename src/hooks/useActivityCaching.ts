...
interface ActivityCache {
  ...
  filteredResults: Map<string, CacheEntry<(ActivityLibraryItem | DiscoveredActivity)[]>>;
}
...
export const useActivityCaching = () => {
  ...
  const getCachedActivities = useCallback((): (ActivityLibraryItem | DiscoveredActivity)[] | null => {
  ...
  const cacheFilteredResults = useCallback((
    ...
    setCache(prev => {
      ...
      if (newFilteredResults.size >= MAX_FILTER_CACHE_SIZE) {
        const oldestKey = Array.from(newFilteredResults.entries())
  ...
  useEffect(() => {
    const cleanupInterval = setInterval(() => {
      ...
      setCache(prev => {
        ...
        return {
          activities: prev.activities && now < prev.activities.expiresAt ? prev.activities : null,
          filteredResults: newFilteredResults
        };
      });
