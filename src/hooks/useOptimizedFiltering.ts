...
const searchActivity = (
  ...
  if (activity.materials?.some(material => searchRegex.test(material))) return true;
...
export const useOptimizedFiltering = (
  ...
  const filteredActivities = useMemo(() => {
    // Check cache first
    const cached = getCachedFilteredResults(searchQuery, selectedPillar, selectedDifficulty);
    if (cached) {
      return cached;
    }
    ...
    if (selectedPillar !== 'all') {
      filtered = filtered.filter(activity => activity.pillar === selectedPillar);
    }
    ...
    if (searchRegex) {
      filtered = filtered.filter(activity => searchActivity(activity, searchRegex));
    }

    // Cache the results
    cacheFilteredResults(searchQuery, selectedPillar, selectedDifficulty, filtered);
