...
import { ChevronDown, ChevronUp, Activity, Clock, AlertTriangle, CheckCircle } from 'lucide-react';
...
const LoadingDiagnosticPanel: React.FC = () => {
  ...
  useEffect(() => {
    const unsubscribe = loadingDiagnosticService.subscribe((newStages, newMetrics) => {
      setStages(newStages);
              ...
              <div>
                ...
                <div className="space-y-1">
                  {stages.slice(-10).map((stage, index) => (
                    <div key={`${stage.name}-${index}`} className="flex items-center justify-between text-xs p-2 bg-gray-50 rounded">
                      ...
                      <div className="flex items-center space-x-2">
                        {stage.duration && (
                          <span className="text-gray-600">{stage.duration.toFixed(0)}ms</span>
                        )}
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    {metrics.slice(-5).map((metric, index) => (
                      <div key={index} className="flex items-center justify-between text-xs p-1 bg-blue-50 rounded">
