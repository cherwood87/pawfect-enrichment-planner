...
const TodaysEnrichmentSummary: React.FC<TodaysEnrichmentSummaryProps> = ({ onChatOpen }) => {
  const navigate = useNavigate();
  const { currentDog } = useDog();
  const { scheduledActivities, getCombinedActivityLibrary, userActivities, discoveredActivities, updateScheduledActivity } = useActivity();
  ...
  const getPillarColor = (pillar: string) => {
    switch (pillar) {
      case 'mental': return 'bg-purple-100 text-purple-700 border-purple-300';
      case 'physical': return 'bg-emerald-100 text-emerald-700 border-emerald-300';
      case 'social': return 'bg-cyan-100 text-cyan-700 border-cyan-300';
      case 'environmental': return 'bg-teal-100 text-teal-700 border-teal-300';
      case 'instinctual': return 'bg-amber-100 text-amber-700 border-amber-300';
      default: return 'bg-gray-100 text-gray-700 border-gray-300';
    }
  };
    ...
    <>
          ...
          <div className="space-y-4">
            {todaysActivities.map(activity => {
              ...
              return (
                <div
                  key={activity.id}
                  className={`modern-card p-4 rounded-2xl transition-all duration-300 hover:shadow-lg ${
                    activity.completed 
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex space-x-2 pt-2">
                      <Button
                        ...
                        className="flex-1 rounded-xl border-purple-300 text-purple-700 hover:bg-purple-50"
                      >
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => handleNeedHelp(activity, activityDetails)}
                        className="flex-1 rounded-xl border-cyan-300 text-cyan-700 hover:bg-cyan-50"
                      >
      ...
      {selectedActivity && (
        <ConsolidatedActivityModal
          ...
          activityDetails={getActivityDetails(selectedActivity.activityId)}
