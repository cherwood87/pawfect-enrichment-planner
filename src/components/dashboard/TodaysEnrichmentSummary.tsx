...
const TodaysEnrichmentSummary: React.FC<TodaysEnrichmentSummaryProps> = ({
  ...
  const [chatContext, setChatContext] = useState<ActivityHelpContext | null>(
    null,
  );
  ...
  const todaysActivities = scheduledActivities.filter(
    (activity) =>
      activity.dogId === currentDog?.id && activity.scheduledDate === today,
  );
  ...
  const handleToggleCompletion = useCallback(
    async (activityId: string, completionNotes?: string) => {
        ...
        {todaysActivities.length === 0 ? (
          <div className="text-center py-8 bg-gradient-to-br from-purple-50 to-cyan-50 rounded-2xl border-2 border-purple-200">
            ...
            <p className="text-sm text-purple-600 mb-4">
              Plan some enrichment activities for {currentDog.name}.
            </p>
            <Button
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      <h3
                        className={`font-semibold text-gray-800 mb-2 ${activity.completed ? "line-through opacity-70" : ""}`}
                      >
                        {activityDetails.title}
                      </h3>
                      <div className="flex flex-wrap gap-2 mb-2">
                        <Badge
                          className={`text-xs border font-semibold ${getPillarColor(activityDetails.pillar)}`}
                        >
                          {activityDetails.pillar}
                        </Badge>
                        <Badge
                          className={`text-xs border font-semibold ${getDifficultyColor(activityDetails.difficulty)}`}
                        >
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex space-x-2 pt-2">
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          handleNeedHelp(activity, activityDetails)
                        }
