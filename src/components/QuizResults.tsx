...
const QuizResultsComponent: React.FC<QuizResultsProps> = ({
	...
	const getScorePercentage = (score: number) => {
		const maxScore = 12; // 4 questions * 3 points
		return (score / maxScore) * 100;
	};

	return (
		<div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30">
			<Card className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl border border-gray-200 shadow-xl bg-white">
						...
						<div className="space-y-3">
							{results.ranking.map((item) => {
								...
								return (
									<div
										key={item.pillar}
										className="flex items-start space-x-4 p-3 rounded-lg bg-white shadow-sm border border-gray-100"
									>
										<div
