		...
		return (
			<div className="mobile-card bg-gradient-to-r from-blue-50 to-orange-50 rounded-lg">
				<div className="flex items-center space-x-2 mb-2">
					<Trophy
						className={`${isMobile ? "w-3 h-3" : "w-4 h-4"} text-orange-500`}
					/>
					<span className="text-sm font-medium text-gray-800">
						Today's Goals for {currentDog.name}
					</span>
				</div>
				<p className="text-sm text-gray-700 mb-2">
					Focus on{" "}
					<span className="font-medium">{topPillars?.[0]?.pillar}</span> and{" "}
					<span className="font-medium">{topPillars?.[1]?.pillar}</span>{" "}
					activities
				</p>
				<div className="flex items-center space-x-2 mb-2">
					...
					<span className="text-xs text-gray-600 flex-shrink-0">
						{totalCompletedToday}/{totalGoalsToday}
					</span>
				</div>
				<Button
					variant="ghost"
