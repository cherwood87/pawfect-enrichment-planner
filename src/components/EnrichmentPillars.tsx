import { Brain, Target, TreePine, Users, Zap } from "lucide-react";
...
const EnrichmentPillars: React.FC<EnrichmentPillarsProps> = ({
	onPillarSelect,
	userPreferences,
}) => {
		...
		<Card>
			<CardHeader className="mobile-card">
				<div className="flex items-center justify-between">
					<CardTitle className="font-bold text-gray-800 truncate">
						Enrichment Pillars
					</CardTitle>
					{userPreferences && (
						<Badge
				...
				{sortedPillars.map((pillar, index) => {
					...
					return (
						...
						>
							...
							>
								<div
									className={`flex items-center ${isMobile ? "flex-col text-center space-y-2" : "space-x-3 flex-1"}`}
								>
									<div
										className={`${isMobile ? "w-8 h-8" : "w-10 h-10"} bg-white rounded-full flex items-center justify-center shadow-sm relative flex-shrink-0`}
									>
										<IconComponent
											className={`${isMobile ? "w-4 h-4" : "w-5 h-5"} text-${pillar.color}-600`}
										/>
										{userPreferences && (
											<div
												className={`absolute -top-1 -right-1 ${isMobile ? "w-4 h-4 text-[10px]" : "w-5 h-5 text-xs"} bg-gray-700 text-white rounded-full flex items-center justify-center font-bold`}
											>
												{userPreference?.rank}
											</div>
										)}
									...
									>
										<div
											className={`flex items-center ${isMobile ? "justify-center flex-col space-y-1" : "space-x-2"}`}
										>
											...
											{isTopPriority && (
												<Badge
													variant="secondary"
													className="text-xs bg-orange-100 text-orange-700 flex-shrink-0"
												>
													Priority
												</Badge>
											)}
								...
								<div
									className={`${isMobile ? "text-center w-full" : "text-right flex-shrink-0"}`}
								>
									<Badge
										variant="secondary"
										className="bg-white/70 text-gray-700 mb-1"
									>
										{pillar.todayCount}/{pillar.weeklyGoal}
									</Badge>
									<div
										className={`${isMobile ? "mx-auto w-20" : "w-16"} bg-white/50 rounded-full h-1.5`}
									>
										<div
