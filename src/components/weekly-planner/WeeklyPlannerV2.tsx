...
import { useActivityData } from '@/contexts/ActivityStateContextV2';
...
interface WeeklyPlannerV2Props {
  onPillarSelect: (pillar: string) => void;
  onChatOpen?: () => void;
}

const WeeklyPlannerV2: React.FC<WeeklyPlannerV2Props> = ({ onPillarSelect }) => {
  // Use the optimized data context
  const { isLoading } = useActivityData();
  ...
  const { getActivityDetails, handleToggleCompletion, isRetrying } = useWeeklyPlannerActions();
  ...
  // Memoize the day card click handler
  ...
  }, [weeklyActivities, getActivityDetails, openModal, onPillarSelect, navigateToActivityLibrary]);

  // Memoize week completion calculation
  ...
  }), [selectedActivityModal, closeModal, handleToggleCompletion]);
        ...
        <CardContent className="grid gap-4">
          ...
          {isRetrying && (
            <div className="flex items-center justify-center py-2">
              ...
              <span className="text-sm text-muted-foreground">Updating activity...</span>
            </div>
          )}
        </CardContent>

        <ConsolidatedActivityModal {...modalProps} />
      </Card>
    </ErrorBoundary>
  );
};

export default WeeklyPlannerV2;
