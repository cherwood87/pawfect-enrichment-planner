...
import { Calendar, ChevronDown, ChevronUp, Trash2, MessageSquare, Clock } from 'lucide-react';
...
const JournalHistory: React.FC<JournalHistoryProps> = ({ dogId, dogName }) => {
  ...
  const handleDeleteEntry = async (entryId: string) => {
    if (!entryId || !window.confirm('Are you sure you want to delete this journal entry?')) return;
                        ...
                        <div className="flex items-center space-x-2">
                          {entry.entryTimestamp && (
                            <div className="flex items-center space-x-1 text-gray-500">
                              ...
                              <span className="text-xs">{formatTime(entry.entryTimestamp)}</span>
                            </div>
                          )}
                          {entry.mood && (
