...
import { Brain, Zap, Users, TreePine, Target, Trophy, RefreshCw, Star } from 'lucide-react';
...
const QuizResultsComponent: React.FC<QuizResultsProps> = ({ 
  ...
  const getScorePercentage = (score: number) => {
    const maxScore = 12; // Based on 4 questions per pillar with max weight of 3
  ...
  return (
    <Card className="max-w-2xl mx-auto border border-gray-200 shadow-xl rounded-3xl">
        ...
        <div className="mt-4">
          ...
          {topPillar && (
            <div className="mt-3 flex items-center justify-center text-sm text-gray-600">
              ...
              <span>Strongest in {pillarNames[topPillar.pillar as keyof typeof pillarNames]} enrichment</span>
            </div>
          )}
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-6">
        <div>
          <h4 className="font-medium text-gray-800 mb-4">Enrichment Pillar Rankings</h4>
          <div className="space-y-3">
            {results.ranking.map((item, index) => {
              ...
              return (
                <div key={item.pillar} className="flex items-start space-x-4 p-4 rounded-lg bg-white shadow-sm border border-gray-100">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-gray-600 mb-2 italic">{item.reason}</div>
                    <div className="text-xs text-gray-500 mb-2 leading-relaxed">{description}</div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {results.recommendations.map((rec, index) => (
              <Badge key={index} variant="outline" className="text-xs p-2 text-center justify-center">
                {rec}
              </Badge>
            ))}
          </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <p className="text-sm text-blue-700">
            Based on {dogName}'s results, focus on <strong>{pillarNames[topPillar?.pillar as keyof typeof pillarNames]}</strong> activities 
            to keep them engaged and happy. You can always retake this quiz as your dog's preferences change!
          </p>
        </div>
