...
import { Send, Loader2, MessageCircle, Lightbulb, Target, TrendingUp, HelpCircle, Save, Download } from 'lucide-react';
...
const ChatModal: React.FC<ChatModalProps> = ({ isOpen, onClose, chatContext }) => {
  ...
  // Download activity as text file (for general chat mode)
  const handleDownloadActivity = (activity: any) => {
    ...
    try {
      ...
      const textContent = `
Enrichment Activity: ${activity.title}

Dog: ${currentDog.name}
Pillar: ${activity.pillar}
Difficulty: ${activity.difficulty}
Duration: ${activity.duration} minutes
Energy Level: ${activity.energyLevel || 'Medium'}

${activity.materials && activity.materials.length > 0 ? `Materials Needed:
${activity.materials.map((material: string) => `- ${material}`).join('\n')}

` : ''}${activity.instructions && activity.instructions.length > 0 ? `Instructions:
${activity.instructions.map((instruction: string, index: number) => `${index + 1}. ${instruction}`).join('\n')}

` : ''}${activity.benefits ? `Benefits:
${activity.benefits}

` : ''}${activity.emotionalGoals && activity.emotionalGoals.length > 0 ? `Emotional Goals:
${activity.emotionalGoals.join(', ')}

` : ''}${activity.tags && activity.tags.length > 0 ? `Tags:
${activity.tags.join(', ')}

` : ''}Generated by Enrichment Coach
Date: ${new Date().toLocaleDateString()}
      ...
      link.href = url;
      link.download = `${activity.title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_activity.txt`;
      ...
      toast({
        title: "Activity Downloaded!",
        description: `"${activity.title}" has been saved as a text file.`,
      });
      ...
      toast({
        ...
        description: "There was an error downloading the activity. Please try again.",
        variant: "destructive",
      });
    }
  };

  // Add to Favourites (localStorage example) - kept for activity help mode
                          ...
                          {message.activities.map((activity, i) => (
                            <div key={i} className="flex items-center justify-between bg-white border rounded-md p-2 shadow-sm">
                              ...
                              {isActivityHelp ? (
                                ...
                                <Button
                                  size="sm"
                                  onClick={() => handleDownloadActivity(activity)}
                                  className="flex items-center gap-1"
                                >
