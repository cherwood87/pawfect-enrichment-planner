...
export const useActivityModalHandlers = (
	...
	const handleActivitySelect = async (activity: any) => {
		console.log(
			"🎯 [ActivityModalHandlers] Activity selected for scheduling:",
			{
		...
		if (!dateResult.isValid) {
			console.error(
				"❌ [ActivityModalHandlers] Invalid date calculated:",
				dateResult.error,
			);
			toast({
				title: "Invalid Date",
		...
		try {
			...
			console.error(
				"❌ [ActivityModalHandlers] Error scheduling activity:",
				error,
			);
			...
			if (
				...
				toast({
					...
					description:
						"This activity is already scheduled for the selected date. The existing schedule has been updated instead.",
					variant: "default",
				});
