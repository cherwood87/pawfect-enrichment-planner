...
export class ActivitySupabaseAdapter extends BaseSupabaseAdapter {
	static async getScheduledActivities(
		...
		console.log(
			"📅 Loading scheduled activities with enhanced adapter for dog:",
			dogId,
		);
		...
		const activities = await ActivitySupabaseAdapter.executeWithRetry(
			async () => {
				console.log("🔍 Querying scheduled activities from Supabase...");
				const { data, error } =
					await ActivitySupabaseAdapter.getSupabaseClient()
				...
				if (error) {
					...
					throw new Error(
						`Failed to fetch scheduled activities: ${error.message}`,
					);
				}
		...
		if (activities.length >= 0) {
			CacheService.cacheActivities(
				`scheduled_${dogId}`,
				activities,
				ActivitySupabaseAdapter.CACHE_DURATION.ACTIVITIES,
			);
		}
	...
	static async getUserActivities(
		...
		console.log(
			"👤 Loading user activities with enhanced adapter for dog:",
			dogId,
		);
		...
		if (activities.length >= 0) {
			CacheService.set(
				`user_activities_${dogId}`,
				activities.map(ActivityDataMapper.mapToUserActivity),
				{
					ttl: ActivitySupabaseAdapter.CACHE_DURATION.USER_ACTIVITIES,
					persistent: true,
				},
			);
		}
	...
	static async createScheduledActivity(
		...
		const result = await ActivitySupabaseAdapter.executeWithRetry(
			async () => {
				const { data, error } =
					await ActivitySupabaseAdapter.getSupabaseClient().rpc(
				...
				if (error) {
					...
					throw new Error(
						`Failed to create scheduled activity: ${error.message}`,
					);
				}
