...
import type { PillarPreference, UserPreference } from "@/types/learning";
...
export class PreferencesService {
	...
	static async getPreferences(
		...
		return data.map((item) => ({
			...
			userId: item.user_id,
			dogId: item.dog_id,
			preferenceType: item.preference_type as
				| "pillar_weights"
	...
	static async calculatePillarPreferences(
		...
		const { data, error } = await supabase.rpc("calculate_pillar_preferences", {
			p_user_id: userId,
			p_dog_id: dogId,
		});
