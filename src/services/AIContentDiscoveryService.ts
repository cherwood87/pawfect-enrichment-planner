...
export class AIContentDiscoveryService {
	...
	static async discoverNewActivities(
		...
		const fullConfig = {
			...AIContentDiscoveryService.DEFAULT_CONFIG,
			...config,
		};
		...
		if (AIContentDiscoveryService.lastDiscoveryAttempt) {
			const timeSinceLastAttempt =
				Date.now() -
				new Date(AIContentDiscoveryService.lastDiscoveryAttempt).getTime();
			if (
				timeSinceLastAttempt <
				AIContentDiscoveryService.MIN_DISCOVERY_INTERVAL_MS
			) {
				console.log(
					"Discovery attempted too recently, skipping to prevent loops",
				);
				return [];
			}
		}
		AIContentDiscoveryService.lastDiscoveryAttempt = now;
		...
		try {
			...
			const discoveredActivities = data.activities.map(
				(activity: any, _index: number) =>
					AIContentDiscoveryService.convertToDiscoveredActivity(activity, true), // Auto-approve all
			);

			console.log(
				`Auto-approved ${discoveredActivities.length} AI-discovered activities`,
			);

			return discoveredActivities.slice(
				0,
				fullConfig.maxActivitiesPerDiscovery,
			);
		} catch (error) {
	...
	private static convertToDiscoveredActivity(
		...
		return {
			...
			instructions: aiActivity.instructions || [
				"Follow the activity description",
			],
			benefits:
				aiActivity.benefits || "AI-curated enrichment activity for your dog.",
	...
	static getDefaultConfig(): ContentDiscoveryConfig {
		return { ...AIContentDiscoveryService.DEFAULT_CONFIG };
	}
	...
	static async scheduleWeeklyDiscovery(
		dogId: string,
		_dogProfile: any,
	): Promise<void> {
