...
export class DiscoveryDomainService {
	...
	static getCombinedActivityLibrary(
	...
	static async discoverNewActivities(
		...
		try {
			...
			const newActivities =
				await AIContentDiscoveryService.discoverNewActivities(
					existingActivities,
			...
			if (newActivities.length > 0) {
				...
				await DiscoveryRepository.saveDiscoveredActivities(
					currentDog.id,
					updatedDiscovered,
				);
				...
				await DiscoveryRepository.saveDiscoveryConfig(
					currentDog.id,
					updatedConfig,
				);

				console.log(
					`AI Discovery complete! Found ${newActivities.length} new activities for ${currentDog.name}`,
				);
				return newActivities;
			} else {
				console.log(
					"No new unique activities found in this AI discovery session",
				);
