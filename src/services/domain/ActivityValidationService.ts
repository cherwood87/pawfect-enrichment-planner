...
export class ActivityValidationService {
	static validateScheduledActivityData(
		...
		ActivityValidationService.validateScheduledDate(activity.scheduledDate);
	}
	...
	static validateScheduledDate(scheduledDate: string): void {
		...
		if (date < yesterday) {
			throw new AppError(
				"Cannot schedule activities for past dates",
				"VALIDATION_ERROR",
			);
		}
	}

	static validateUserActivityData(
	...
	static async checkForDuplicateActivity(
		activity: Omit<ScheduledActivity, "id">,
	): Promise<void> {
		try {
			const existingActivities =
				await ActivityRepository.getScheduledActivities(activity.dogId);
