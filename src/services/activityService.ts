...
export class ActivityService {
	...
	static async deleteScheduledActivity(_id: string): Promise<void> {
		// This will need to be implemented in the domain service
		throw new Error(
			"Delete functionality needs to be implemented in domain service",
		);
	}
	...
	static async getUserActivities(dogId: string): Promise<UserActivity[]> {
		try {
			...
			console.error(
				"Failed to load user activities from Supabase, trying localStorage fallback:",
				error,
			);
	...
	static async migrateScheduledActivitiesFromLocalStorage(
		dogId: string,
	): Promise<void> {
		try {
			...
			console.error(
				"Failed to migrate scheduled activities for dog:",
				dogId,
				error,
			);
		}
	...
	static async migrateUserActivitiesFromLocalStorage(
		dogId: string,
	): Promise<void> {
		try {
			await ActivityRepository.migrateFromLocalStorage(dogId);
