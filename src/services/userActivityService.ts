...
import { ActivityMappers, DatabaseUserActivity } from './mappers/activityMappers';
...
export class UserActivityService {
  ...
  static async create(activity: Omit<UserActivity, 'id' | 'createdAt'>): Promise<UserActivity> {
    const { data, error } = await supabase
      .from('user_activities')
