...
export class ActivitySyncService {
	...
	static async syncCuratedActivities(): Promise<SyncResult> {
		try {
			...
			const activitiesToSync = activityLibrary.map((activity) =>
				ActivitySyncService.transformToSupabaseActivity(activity, "curated"),
			);
	...
	static async syncDiscoveredActivities(
		...
		try {
			...
			const activitiesToSync = discoveredActivities.map((activity) =>
				ActivitySyncService.transformToSupabaseActivity(
					activity,
					"discovered",
					dogId,
				),
	...
	static async fullSync(
		...
		try {
			...
			const curatedResult = await ActivitySyncService.syncCuratedActivities();
			...
			if (dogId) {
				discoveredResult = await ActivitySyncService.syncDiscoveredActivities(
					discoveredActivities,
					dogId,
				);
				userResult = await ActivitySyncService.syncUserActivities(
					userActivities,
					dogId,
				);
			}
