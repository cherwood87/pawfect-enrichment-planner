...
export class ActivitySyncService {
  private static transformToSupabaseActivity(
    ...
    if (source === 'discovered') {
      ...
      return {
        ...
        discovered_at: discoveredActivity.discoveredAt || new Date().toISOString(),
  ...
  static async fullSync(
    ...
    try {
      ...
      if (dogId) {
        discoveredResult = await this.syncDiscoveredActivities(discoveredActivities, dogId);
  ...
  static async loadActivitiesFromSupabase(dogId?: string): Promise<{
    ...
    try {
      ...
      if (dogId) {
        query = query.or(`source.eq.curated,and(source.neq.curated,dog_id.eq.${dogId})`);
      } else {
