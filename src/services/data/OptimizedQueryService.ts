...
class OptimizedQueryService {
  ...
  async getDashboardData(
    ...
    try {
      ...
      const result: DashboardData = {
        dog:
          dogResult.status === "fulfilled" && dogResult.value
            ? DogDataMapper.mapToDog(dogResult.value)
            : null,
        scheduledActivities:
          scheduledResult.status === "fulfilled" && scheduledResult.value
            ? scheduledResult.value.map(
                ActivityDataMapper.mapToScheduledActivity,
              )
            : [],
        userActivities:
          userActivitiesResult.status === "fulfilled" &&
          userActivitiesResult.value
            ? userActivitiesResult.value.map(
                ActivityDataMapper.mapToUserActivity,
              )
            : [],
        recentCompletions:
  ...
  async getWeeklyPlannerData(
    ...
    try {
      console.log(
        "🔍 Fetching weekly planner data for:",
        dogId,
        startDate,
        endDate,
      );
  ...
  async createScheduledActivitiesBatch(
    activities: Omit<ScheduledActivity, "id" | "created_at" | "updated_at">[],
  ): Promise<ScheduledActivity[]> {
    try {
      console.log(
        "💾 Creating scheduled activities batch:",
        activities.length,
        "activities",
      );
      ...
      const createdActivities = result.map(
        ActivityDataMapper.mapToScheduledActivity,
      );
      console.log(
        "✅ Batch activities created successfully:",
    ...
    try {
      ...
      const result = await RetryService.executeWithRetry(
        async () => {
          ...
          if (searchTerm) {
            query = query.or(
              `title.ilike.%${searchTerm}%,benefits.ilike.%${searchTerm}%`,
            );
          }
      ...
      console.log(
        "✅ Activity library fetched:",
        libraryResult.data.length,
        "activities, total:",
        libraryResult.total,
      );
  ...
  private isValidLibraryData(
    data: any,
  ): data is { data: any[]; total: number } {
    return (
      data &&
      typeof data === "object" &&
      "data" in data &&
