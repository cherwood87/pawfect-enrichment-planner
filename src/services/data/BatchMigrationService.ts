...
export class BatchMigrationService {
	...
	static async batchMigrateScheduledActivities(dogId: string): Promise<{
		...
		console.log(
			`🔄 Starting batch migration of scheduled activities for dog ${dogId}...`,
		);
		...
		try {
			...
			for (
				let i = 0;
				i < localActivities.length;
				i += BatchMigrationService.BATCH_SIZE
			) {
				const batch = localActivities.slice(
					i,
					i + BatchMigrationService.BATCH_SIZE,
				);
				...
				const batchPromises = batch.map(async (activity) => {
					try {
						...
						handleError(
							errorObj,
							{ operation: "batchMigrateScheduledActivity", activity },
							false,
						);
				...
				if (i + BatchMigrationService.BATCH_SIZE < localActivities.length) {
			...
			const successRate =
				results.migrated / (results.migrated + results.failed);
			if (successRate > 0.8) {
				LocalStorageAdapter.clearScheduledActivities(dogId);
				console.log(
					`✅ Batch migration completed for dog ${dogId}. Migrated: ${results.migrated}, Failed: ${results.failed}`,
				);
			} else {
				console.warn(
					`⚠️ Migration partially failed for dog ${dogId}. Success rate: ${(successRate * 100).toFixed(1)}%`,
				);
			...
			handleError(errorObj, {
				operation: "batchMigrateScheduledActivities",
				dogId,
			});
	...
	static async batchMigrateUserActivities(dogId: string): Promise<{
		...
		console.log(
			`🔄 Starting batch migration of user activities for dog ${dogId}...`,
		);
			...
			for (
				...
				const batchPromises = batch.map(async (activity) => {
					try {
						...
						handleError(
							errorObj,
							{ operation: "batchMigrateUserActivity", activity },
							false,
						);
						return { success: false, error: errorObj };
					}
