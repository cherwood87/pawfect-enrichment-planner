...
export class ScheduledActivityRepository extends BaseRepository {
  ...
  static async getScheduledActivities(
    ...
    try {
      ...
      try {
        const activities = await this.optimizedQuery.getWeeklyPlannerData(
          dogId,
          today,
          oneMonthFromNow,
        );
  ...
  static async createScheduledActivitiesBatch(
    ...
    try {
      ...
      activities.forEach((activity) =>
        this.validateScheduledActivityForBatch(activity),
      );

      // Use optimized query service for batch operations
      const created =
        await this.optimizedQuery.createScheduledActivitiesBatch(activities);
