...
export class DogService {
	...
	private static async getCurrentUser() {
		...
		if (DogService._currentUser) {
			return DogService._currentUser;
		}

		// Reuse existing promise if one is in flight
		if (DogService._userPromise) {
			return DogService._userPromise;
		}

		// Create new promise and cache it
		DogService._userPromise = supabase.auth
			.getUser()
			.then(({ data: { user } }) => {
				DogService._currentUser = user;
				DogService._userPromise = null; // Clear promise cache
				return user;
			});
	...
	static clearUserCache() {
		DogService._currentUser = null;
		DogService._userPromise = null;
	}

	static async createDog(
		dogData: Omit<Dog, "id" | "dateAdded" | "lastUpdated">,
	): Promise<Dog> {
		const user = await DogService.getCurrentUser();
		if (!user) {
		...
		return DogService.mapToDog(data);
	}
	...
	static async getAllDogs(): Promise<Dog[]> {
		...
		return (data || []).map((dog) => DogService.mapToDog(dog));
	}
	...
	static async migrateFromLocalStorage(): Promise<void> {
		try {
			...
			console.log(
				`Migrating ${dogs.length} dogs to Supabase for user ${user.id}...`,
			);
			...
			console.log(
				`Migration completed: ${successful}/${dogs.length} dogs migrated successfully`,
			);
