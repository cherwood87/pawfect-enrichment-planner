...
export class DeploymentHealthChecker {
  ...
  private async checkApplicationLoad(): Promise<DeploymentHealthReport['checks'][string]> {
    const startTime = Date.now();
  ...
  private async checkNetworkConnectivity(): Promise<DeploymentHealthReport['checks'][string]> {
    const startTime = Date.now();
  ...
  private async checkSupabaseConnection(): Promise<DeploymentHealthReport['checks'][string]> {
    const startTime = Date.now();
  ...
  private async checkStaticAssets(): Promise<DeploymentHealthReport['checks'][string]> {
    const startTime = Date.now();
    ...
    try {
      ...
      const failedAssets = assetChecks.filter(result => result.status === 'rejected');
...
if (process.env.NODE_ENV === 'production') {
  ...
  setTimeout(() => {
    deploymentHealthChecker.performHealthCheck().then(report => {
