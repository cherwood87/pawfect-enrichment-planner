...
export class DuplicateDetector {
	...
	static checkTimeConflicts(
		_newActivity: Partial<ScheduledActivity>,
		_existingActivities: ScheduledActivity[],
	): DuplicateCheckResult {
	...
	static performComprehensiveCheck(
		...
		const exactCheck = DuplicateDetector.checkExactDuplicate(
			newActivity,
			existingActivities,
		);
		...
		// Check daily limits
		const dailyCheck = DuplicateDetector.checkDailyLimit(
			newActivity,
			existingActivities,
			options.maxPerDay,
		);
		...
		const timeCheck = DuplicateDetector.checkTimeConflicts(
			newActivity,
			existingActivities,
		);
