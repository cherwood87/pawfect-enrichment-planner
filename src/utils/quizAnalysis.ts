...
export const generateReason = (pillar: string, score: number): string => {
  const reasons = {
    mental:
      score >= 4
        ? "High problem-solving interest"
        : score >= 2
          ? "Moderate mental stimulation needs"
          : "Prefers simple activities",
    physical:
      score >= 4
        ? "High energy, needs lots of exercise"
        : score >= 2
          ? "Moderate activity needs"
          : "Low energy, gentle exercise preferred",
    social:
      score >= 4
        ? "Very social, loves interaction"
        : score >= 2
          ? "Enjoys social time with guidance"
          : "Prefers quiet, familiar company",
    environmental:
      score >= 4
    ...
    instinctual:
      score >= 4
        ...
        : score >= 2
          ? "Some instinctual interests"
          : "Limited instinctual behaviors",
  };
  return reasons[pillar as keyof typeof reasons] || "Needs assessment";
};

export const determinePersonality = (
  scores: Record<string, number>,
): string => {
  const maxScore = Math.max(...Object.values(scores));
  const maxPillar = Object.entries(scores).find(
  ...
  return (
    personalities[maxPillar as keyof typeof personalities] ||
    "Balanced Companion"
  );
};
...
export const generateRecommendations = (
  ...
  return topPillars
    .flatMap((pillar) => recommendations[pillar.pillar]?.slice(0, 2) || [])
    .slice(0, 6);
};
