...
export class DataValidator {
	...
	static validateScheduledActivity(
		...
		if (
			activity.reminderEnabled !== undefined &&
			typeof activity.reminderEnabled !== "boolean"
		) {
	...
	static validateUserActivity(
		...
		if (!activity.pillar) {
			...
			if (!validPillars.includes(activity.pillar)) {
				errors.push(
					`Invalid pillar: ${activity.pillar}. Must be one of: ${validPillars.join(", ")}`,
				);
			}
		}
		...
		if (!activity.difficulty) {
			...
			if (!validDifficulties.includes(activity.difficulty)) {
				errors.push(
					`Invalid difficulty: ${activity.difficulty}. Must be one of: ${validDifficulties.join(", ")}`,
				);
			}
		}
		...
		if (activity.instructions && activity.instructions.length > 100) {
			warnings.push(
				"Many instruction steps - consider breaking into smaller activities",
			);
		}
	...
	static validateDog(dog: Partial<Dog>): ValidationResult {
		...
		if (dog.activityLevel) {
			...
			if (!validActivityLevels.includes(dog.activityLevel)) {
				errors.push(
					`Invalid activity level: ${dog.activityLevel}. Must be one of: ${validActivityLevels.join(", ")}`,
				);
			}
		}
	 ...
	 */
	static validateDiscoveredActivity(
		activity: Partial<DiscoveredActivity>,
	): ValidationResult {
		const errors: string[] = [];
		const warnings: string[] = [];

		// Use base user activity validation
		const baseValidation = DataValidator.validateUserActivity(activity);
		...
		if (activity.qualityScore !== undefined) {
			if (
				typeof activity.qualityScore !== "number" ||
				...
				warnings.push(
					"Quality score is low - consider reviewing before approval",
				);
			}
		}
	...
	static validateBulkOperation<T>(
		...
		if (items.length > maxItems) {
			throw new Error(
				`Bulk operation exceeds maximum limit of ${maxItems} items`,
			);
		}
 ...
 */
export const normalizeActivityData = (
	activity: Partial<ScheduledActivity>,
): Partial<ScheduledActivity> => {
	return {
		...activity,
		notes: DataValidator.sanitizeInput(activity.notes || ""),
		completionNotes: DataValidator.sanitizeInput(
