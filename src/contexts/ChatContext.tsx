...
export const ChatProvider: React.FC<{ children: ReactNode }> = ({
	...
	const loadConversation = (
		...
		const existing = conversations.find(
			(conv) => conv.dogId === conversationKey,
		);
	...
	const startNewConversation = (type: ConversationType = "general") => {
		...
		console.log(
			"Starting new conversation for dog:",
			currentDog.name,
			"type:",
			type,
		);
		...
		if (type === "general") {
			setConversations((prev) => [
				...prev.filter((conv) => conv.dogId !== conversationKey),
				newConversation,
			]);
		}
	};
	...
	const sendMessage = async (content: string, activityContext?: any) => {
		...
		try {
			...
			const isGeneralChat =
				!currentConversation.dogId.includes("-activity-help");
			if (isGeneralChat) {
				setConversations((prev) =>
					prev.map((conv) =>
			...
			const { data, error } = await supabase.functions.invoke(
				"enrichment-coach",
				{
					body: {
						...
						activityHistory: {
							todaysActivities,
							totalActivitiesCompleted: todaysActivities.filter(
								(a) => a.completed,
							).length,
						},
						pillarBalance,
