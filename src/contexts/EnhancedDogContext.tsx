...
export const EnhancedDogProvider: React.FC<{ children: ReactNode }> = ({
	children,
}) => {
	...
	const loadDogs = async (forceRefresh = false) => {
		...
		try {
			...
			console.log(
				"🐕 Loading dogs with enhanced service, forceRefresh:",
				forceRefresh,
			);
			...
			console.log(
				`✅ Enhanced dogs loaded in ${loadTime.toFixed(2)}ms:`,
				dogs.length,
			);
			...
			toast({
				title: "Loading Error",
				description:
					"Failed to load your dogs. Using cached data if available.",
				variant: "destructive",
			});
		} finally {
			dispatch({ type: "SET_LOADING", payload: false });
...
export const useEnhancedDog = () => {
	...
	if (context === undefined) {
		throw new Error(
			"useEnhancedDog must be used within an EnhancedDogProvider",
		);
	}
	return context;
};
