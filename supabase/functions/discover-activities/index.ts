...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `Discovering activities for dog: ${dogProfile?.name || "Unknown"}`,
    );
    console.log(
      `Existing activities count: ${existingActivities?.length || 0}`,
    );
    ...
    const pillarCounts =
      existingActivities?.reduce((acc: any, activity: any) => {
...
${Object.entries(pillarCounts)
  .map(([pillar, count]) => `- ${pillar}: ${count} activities`)
  .join("\n")}
    ...
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          {
            role: "user",
            content: `Generate ${maxActivities} unique enrichment activities for this ${dogProfile?.breed || "dog"} considering their specific needs and avoiding duplicates.`,
          },
        ],
        temperature: 0.8,
        max_tokens: 2000,
